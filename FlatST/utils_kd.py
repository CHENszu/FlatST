...
def _csr_rows_to_dense(X_csr, row_indices):
    """Materialize selected CSR rows without fancy indexing (avoids rare scipy CSR segfaults)."""
    n_feat = X_csr.shape[1]
    out = np.empty((len(row_indices), n_feat), dtype=np.float64)
    for r, idx in enumerate(row_indices):
        out[r] = X_csr.getrow(int(idx)).toarray().ravel()
    return out


def Cal_Spatial_Net(adata, rad_cutoff=None, verbose=True, build_torch_graph=True):
    """If build_torch_graph is False, only fills adata.uns['Spatial_Net'] and returns None.

    Cal_Spatial_Net_3D never uses the returned graph but only the DataFrame; skipping the
    torch.Data avoids torch.FloatTensor(adata.X.todense()), which OOMs on large slice pairs.
    """
    if verbose:
        print('------Calculating spatial graph...')
    coor_arr = np.ascontiguousarray(np.asarray(adata.obsm['spatial']), dtype=np.float64)
    if coor_arr.ndim != 2 or coor_arr.shape[0] != adata.n_obs:
        raise ValueError("adata.obsm['spatial'] must be shape (n_obs, 2).")
    if not np.isfinite(coor_arr).all():
        raise ValueError("adata.obsm['spatial'] contains NaN or inf; fix before Cal_Spatial_Net.")

    # Use ndarray + kd_tree + n_jobs=1: DataFrame/float32/non-contiguous input can trigger
    # native-code issues in NearestNeighbors on large subsamples.
    nbrs = sklearn.neighbors.NearestNeighbors(
        radius=rad_cutoff, algorithm='kd_tree', n_jobs=1
    ).fit(coor_arr)
    distances, indices = nbrs.radius_neighbors(coor_arr, return_distance=True)
    # Avoid pd.concat on ~n_obs tiny DataFrames — very high memory use and can kill the kernel.
    c1_parts, c2_parts, d_parts = [], [], []
    for it in range(indices.shape[0]):
        m = indices[it].shape[0]
        if m == 0:
            continue
        c1_parts.append(np.full(m, it, dtype=np.int64))
        c2_parts.append(np.asarray(indices[it], dtype=np.int64))
        d_parts.append(np.asarray(distances[it], dtype=np.float64))
    if not c1_parts:
        KNN_df = pd.DataFrame(columns=['Cell1', 'Cell2', 'Distance'])
    else:
        KNN_df = pd.DataFrame({
            'Cell1': np.concatenate(c1_parts),
            'Cell2': np.concatenate(c2_parts),
            'Distance': np.concatenate(d_parts),
        })

    Spatial_Net = KNN_df.copy()
    Spatial_Net = Spatial_Net.loc[Spatial_Net['Distance'] > 0,]
    id_cell_trans = dict(zip(range(coor_arr.shape[0]), np.array(adata.obs.index)))
    Spatial_Net['Cell1'] = Spatial_Net['Cell1'].map(id_cell_trans)
    Spatial_Net['Cell2'] = Spatial_Net['Cell2'].map(id_cell_trans)

    # Obtain edge weights from Pearson correlation across genes
    ...
    n_obs = adata.n_obs
    # Full np.corrcoef(dense_X) is O(n^2) memory (~130k cells -> ~100+ GB) and kills the kernel.
    # For large objects, compute correlation only for each spatial edge.
    _CORR_FULL_MATRIX_MAX_OBS = 8192

    def _pearson_two_vectors(x, y):
    ...
    if n_obs <= _CORR_FULL_MATRIX_MAX_OBS:
        dense_X = adata_Vars.X.toarray() if sp.issparse(adata_Vars.X) else np.asarray(adata_Vars.X)
        ...
        for i, j in zip(Spatial_Net['Cell1'], Spatial_Net['Cell2']):
            ...
            edge_weights.append(corr_matrix[i_idx, j_idx])
    else:
        # Avoid full X.toarray() for ~130k cells (~1.2+ GB peak); chunk edges and densify only
        # the union of rows needed per chunk (typically tens of MB peak).
        Xv = adata_Vars.X
        c1 = Spatial_Net['Cell1'].values
        c2 = Spatial_Net['Cell2'].values
        n_edges = len(c1)
        if n_edges == 0:
            edge_weights = []
        else:
            ii_all = np.array([cell_index_dict[c] for c in c1], dtype=np.int64)
            jj_all = np.array([cell_index_dict[c] for c in c2], dtype=np.int64)
            _EDGE_CHUNK = 1024
            w = np.empty(n_edges, dtype=np.float64)
            if sp.issparse(Xv):
                X_csr = Xv.tocsr()
                for start in range(0, n_edges, _EDGE_CHUNK):
                    end = min(start + _EDGE_CHUNK, n_edges)
                    ii = ii_all[start:end]
                    jj = jj_all[start:end]
                    uniq = np.unique(np.concatenate([ii, jj]))
                    idx_map = {int(u): k for k, u in enumerate(uniq)}
                    sub = _csr_rows_to_dense(X_csr, uniq)
                    for k in range(end - start):
                        w[start + k] = _pearson_two_vectors(
                            sub[idx_map[ii[k]]], sub[idx_map[jj[k]]])
            else:
                X_arr = np.asarray(Xv)
                for start in range(0, n_edges, _EDGE_CHUNK):
                    end = min(start + _EDGE_CHUNK, n_edges)
...
def Cal_Spatial_Net_3D(adata, rad_cutoff_2D, rad_cutoff_Zaxis,
    ...
    for temp_section in np.unique(adata.obs[key_section]):
        ...
        Cal_Spatial_Net(
            temp_adata, rad_cutoff=rad_cutoff_2D, verbose=False, build_torch_graph=False)
    ...
    for it in range(num_section - 1):
        ...
        temp_adata = adata[adata.obs[key_section].isin(
            [section_1, section_2]),]
        Cal_Spatial_Net(
            temp_adata, rad_cutoff=rad_cutoff_Zaxis, verbose=False, build_torch_graph=False)
        ...
        temp_adata.uns['Spatial_Net']['Section_id_2'] = temp_adata.uns['Spatial_Net']['Cell2'].map(
            spot_section_trans)
        used_edge = temp_adata.uns['Spatial_Net']['Section_id_1'].ne(
            temp_adata.uns['Spatial_Net']['Section_id_2'])
